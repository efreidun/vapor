...
def plot_ceiling(
    ...
    quat_bing: Optional[np.ndarray] = None,
    H=None,
    ...
    Args:
        ...
        quat_pred: single rotation prediction in quaternion [w, x, y, z], shape (4,)
    ...
    # ax = fig.add_subplot(grid_spec[40:65, 90:100])
    # ax = fig.add_subplot(grid_spec[0:30, 20:40])
    # ax.set_title("marginal histogram on z-axis")
    # ax.hist(
    #     tra_samples[:, 2],
    #     bins=1250 // 100,
    #     range=[-0.5, 0.5],
    ...
    # third row
    # corners = np.array([[0, 0, 1], [639, 0, 1], [639, 479, 1], [0, 479, 1], [0, 0, 1]])
    corners = np.array(
        [
            [img_corner[1], img_corner[0], 1],
            [img_corner[1] + 224, img_corner[0], 1],
            [img_corner[1] + 224, img_corner[0] + 224, 1],
            [img_corner[1], img_corner[0] + 224, 1],
            [img_corner[1], img_corner[0], 1],
        ]
    )
    wrpd_corners = [H @ corner for corner in corners]
    wrpd_corners = [corner[:2] / corner[2] for corner in wrpd_corners]
    xs, ys = zip(*wrpd_corners)
...
def plot_latent(
    ...
    # )
    # pos_fig.savefig(
    #     "/home/fereidoon/figures/latent/positions.png", bbox_inches="tight"
    # )
    # lat_fig = plt.figure(figsize=(10, 10))
    # plot_code_colored_on_plane(
    #     lat_fig,
    #     "111",
    #     None,
    ...
    # )
    # lat_fig.savefig(
    #     "/home/fereidoon/figures/latent/latent.png", bbox_inches="tight"
    # )
...
def plot_posterior(
    ...
    # data_fig = plt.figure(figsize=(15, 10))
    # show_image(
    #     data_fig,
    #     "121",
    #     "",
    ...
    # )
    # data_fig.savefig(
    #     f"/home/fereidoon/figures/blue_chairs/{title[-4:]}.png", bbox_inches="tight"
    # )
    tra_fig = plt.figure(figsize=(10, 10))
    plot_tra_hist_on_plane(
        tra_fig,
        "111",
        None,
    ...
    tra_fig.savefig(
        "/home/fereidoon/figures/new_new_ablation/tra_posterior.png", bbox_inches="tight"
    )
    rot_fig = plt.figure(figsize=(10, 10))
    plot_rot_hist_on_plane(
        rot_fig,
        "111",
        None,
    ...
    rot_fig.savefig(
        "/home/fereidoon/figures/new_new_ablation/rot_posterior.png", bbox_inches="tight"
    )
    render_fig = plt.figure(figsize=(10, 10))
    show_image(
        render_fig,
        "111",
        None,
    ...
    render_fig.savefig("/home/fereidoon/new_new_ablation/constrain/samples.png", bbox_inches="tight")
...
def render_3d(
    ...
    if tra_gt is not None and quat_gt is not None:
        ...
        for tra, quat, color in zip(tra_gt, quat_gt, colors):
            # for tfmat in tfmat_gt:
            for noise in noises:
...
def plot_tra_hist_on_plane(
    ...
    tra_mins = scene_dims[0] - scene_dims[2]
    tra_maxs = scene_dims[1] + scene_dims[2]
    xlim = [tra_mins[0] + 0, tra_maxs[0] - 0]
    ...
    ax.hist2d(*tra_samples.T[[0, 1]], bins=50, range=[xlim, ylim], cmap="plasma")

    colors = np.array([[1, 0, 0], [0, 1, 0], [0, 0.4, 1]])
    if tra_gt is not None:
        # ax.scatter(*tra_gt.T[:2], s=20, color="red")
        for tra, color in zip(tra_gt, colors):
            ax.add_patch(
                Circle(
                    xy=tra[[0, 1]],
                    radius=0.7,
...
def plot_rot_hist_on_plane(
    ...
    if quat_gt is not None:
        ...
        for hopf, color in zip(hopf_gt, colors):
            ax.add_patch(
                Circle(
                    xy=hopf[:2],
                    radius=0.4,
